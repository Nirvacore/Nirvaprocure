...
export default function AnalyticsPage() {
  ...
  const { data, loading, error, refresh } = useResource(
    () => withMockFallback(() => analyticsApi.summary(), async () => (await import('@/lib/dashboard-demo-fixtures')).loadAnalyticsSummary()),
  );

  const { data: aiSummary } = useResource(
    () => withMockFallback(
      () => aiRuns.list(1).then(r => r.summary),
      async () => (await import('@/lib/dashboard-demo-fixtures')).loadAnalyticsAiSummary(),
    ),
  );
  ...
  const { data: risks, loading: risksLoading, refresh: refreshRisks } = useResource(
    () => risksShown
      ? withMockFallback(() => supplierRisk.list(), async () => (await import('@/lib/dashboard-demo-fixtures')).loadAnalyticsRisks())
